...
const product = computed(
  () =>
    products.filter((item) => item.id === Number(props.id))[0] || {
      id: 0,
      title: "Товар не существует",
      price: 0,
      colorsId: [],
      categoryId: 0,
      image: "",
...
const color = ref(product.value.colorsId[0]);
const colors = reactive(colorsData);

const category = computed(
  () =>
    categories.find((category) => category.id === product.value.categoryId)
      ?.title ?? "Нет категории"
);
